...
@Injectable()
export class RequestMaterialsService {
  private readonly logger = new Logger(RequestMaterialsService.name);

  constructor(
    @InjectRepository(RequestMaterials)
    private readonly requestDocumentsRepository: Repository<RequestMaterials>,
  ...
  async create(user: User, createUserTypeDto: CreateRequestMaterialsDto) {
    try {
  ...
  async updated(user: User, createUserTypeDto: CreateRequestMaterialsDto) {
    try {
      const systemDocument = await this.requestDocumentsRepository.save({
        ...createUserTypeDto,
        updated_by: user.id,
